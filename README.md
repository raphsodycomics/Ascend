...
- A 5-minute inactivity timeout on the FA dashboard: after 5 minutes with no activity, an on-screen 50-second countdown appears, and the FA is signed out automatically if it reaches zero — see `startInactivityTimer()` in `js/auth.js` to add this to other roles
...
- **FA inactivity timeout** — after 5 minutes with no mouse/keyboard/touch/scroll
  activity, an on-screen 50-second countdown appears; if it reaches zero the FA is
  signed out and redirected to the login page with an explanatory message. Built
  as a reusable `startInactivityTimer()` export in `js/auth.js` so it can be added
  to any other role's dashboard with two lines (import it, call it once inside
  `guardPage`'s callback) — currently only wired into `fa.html`.
